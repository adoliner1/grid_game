...
class FieldsOfTheFaithfulStatus(Status):
    def __init__(self, duration, player_with_status):
        super().__init__(
            name="Fields of the Faithful",
            description="Recruiting costs are reduced by 1 (Fields of the Faithful)",
            duration=duration,
            player_with_status=player_with_status
        )

    def modify_recruiting_costs(self, game_state):
        for disciple in game_state['recruiting_costs'][self.player_with_status]:
            game_state['recruiting_costs'][self.player_with_status][disciple] = max(1, game_state['recruiting_costs'][self.player_with_status][disciple] - 1)
...
class FieldsOfTheFaithful(Tile):
            ...
            influence_tiers=[
                {
                    ...
                    "must_be_ruler": True,
                    "description": "Reduce your ((recruiting)) costs by 1 for the rest of the round",
    ...
    async def use_a_tier(self, game_state, tier_index, game_action_container_stack, send_clients_log_message, get_and_send_available_actions, send_clients_game_state):
        ...
        fields_status = FieldsOfTheFaithfulStatus(duration="round", player_with_status=user)
