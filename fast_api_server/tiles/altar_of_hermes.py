...
class AltarOfHermes(Tile):
            ...
            influence_tiers=[
                {
                    ...
                    "must_be_ruler": True,                    
                    "description": "**Action:** ^^Burn^^ one of your disciples on another tile for +2 leader_movement",
    ...
    async def use_a_tier(self, game_state, tier_index, game_action_container_stack, send_clients_log_message, get_and_send_available_actions, send_clients_game_state):
        ...
        leader_movement_gained = 2
        game_state['leader_movement'][user] += leader_movement_gained
        await send_clients_log_message(f"{user} gains {leader_movement_gained} leader_movement from **{self.name}**")
