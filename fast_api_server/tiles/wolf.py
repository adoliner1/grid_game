...
class Wolf(Tile):
    def __init__(self):
        super().__init__(
            name="Wolf",
            type="Producer/Giver",
            description="The player with the fewest shapes produces 1 triangle at the start of a round\nRuler, Most Shapes, Action: Once per round, receive 2 squares at an adjacent tile",
            number_of_slots=3,
    ...
    async def start_of_round_effect(self, game_state, game_action_container_stack, send_clients_log_message, send_clients_available_actions, send_clients_game_state):
        ...
        await game_utilities.produce_shape_for_player(game_state, game_action_container_stack, send_clients_log_message, send_clients_available_actions, send_clients_game_state, player_with_fewest_shapes, 1, 'triangle', self.name)
