...
class GameEngine:
    ...
    def set_notify_clients(self, callback: Callable[[str], None]):
        self.notify_clients = callback

    async def run_game_loop(self):  
        while not self.game_has_ended:
            if not self.game_action_container_stack:
                self.game_action_container_stack.push(game_engine_utilities.create_new_turn_game_action_container(self.game_state.whose_turn_is_it))
            
            current_game_action_container = self.game_action_container_stack.pop()

            available_actions = self.get_available_actions(current_game_action_container)



            #await self.send_available_actions()
            #await self.current_event.event.wait()
            
            if current_game_action_container.is_complete():
                await self.execute_game_action()
            else:
                self.event_stack.append(self.current_event)

    def player_takes_place_on_slot_on_powerup_action(local_game_state, player_color, slot_index, tile_index, shape_type):
        pass

    def player_takes_use_powerup_action(local_game_state, powerup_index, player_color, **current_action):
        pass

    def get_available_actions(self, game_action_container):
        ...
        if current_action:
            if current_action["action"] == 'place_shape_on_slot':
                ...
                slots_that_can_be_placed_on = game_utilities.get_slots_that_can_be_placed_on(game_state, shape_type_to_place)
                available_actions_with_details["select_a_slot_on_a_tile"] = slots_that_can_be_placed_on
    ...
    async def start_round(self, game_state):
        ...
        if game_state['first_player']:
            ...
            game_state["whose_action_is_it"] = game_state['first_player']
        else:
        ...
        for tile in game_state["tiles"]:
            await tile.start_of_round_effect(game_state, self.send_log_message)
            tile.is_on_cooldown = False

        #give base income
        await self.give_base_income_to_players(game_state)
        game_engine_utilities.determine_rulers(game_state)
    ...
    async def give_base_income_to_players(self, game_state):
        ...
        for player_color in game_constants.player_colors:
            for shape, amount in game_constants.shapes_to_give:
                await game_utilities.produce_shape_for_player(game_state, player_color, amount, shape, None)
    ...
    async def player_takes_use_tile_action(self, game_state, tile_index, player_color, **data):
        ...
        if not await tile.use_tile(game_state, player_color, self.send_log_message, **data):
            return
        game_engine_utilities.determine_rulers(game_state)
        if game_state["player_has_passed"][game_engine_utilities.determine_rulers(player_color)] == False:
            game_state["whose_turn_is_it"] = game_engine_utilities.determine_rulers(player_color)
            game_state["whose_action_is_it"] = game_engine_utilities.determine_rulers(player_color)
            await self.send_log_message(f"Turn passes to {game_state['whose_turn_is_it']} player")
        else:
    ...
    async def player_takes_place_on_slot_on_tile_action(self, game_state, player_color, slot_index, tile_index, shape_type):
        ...
        await game_engine_utilities.place_shape_on_tile(game_state, tile_index, slot_index, shape_type, player_color, self.send_log_message)
    ...
    async def player_passes(self, game_state, player_color):
        ...
        if game_state["player_has_passed"]["red"] and game_state["player_has_passed"]["blue"]:
            ...
            game_state["whose_turn_is_it"] = game_engine_utilities.determine_rulers(player_color)
            game_state["whose_action_is_it"] = game_engine_utilities.determine_rulers(player_color)
            await self.send_log_message(f"turn passes to {game_state['whose_turn_is_it']}")
    ...
    async def end_round(self, game_state):
        ...
        for tile in game_state["tiles"]:
            await tile.end_of_round_effect(game_state, self.send_log_message)

        for listener_name, listener_function in game_state["listeners"]["end_of_round"].items():
            await listener_function(game_state, self.send_log_message)  
    ...
    async def end_game(self, game_state):
        for tile in game_state["tiles"]:
            await tile.end_of_game_effect(game_state, self.send_log_message)

        await self.send_updated_game_state()
        await self.send_log_message(f"Final Score: Red: {game_state['points']['red']} Blue: {game_state['points']['blue']}")
